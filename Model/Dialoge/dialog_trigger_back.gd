...
func _input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("interaction") and started and not dialoguesLine[current_dialogue] == "[INPUT]" :
		continume_dialoge()

func continume_dialoge() :
	if dialoguesLine[current_dialogue] == "[INPUT]" and typingContainer.text.length() < 5 :
		return
	current_dialogue += 1
	if current_dialogue < dialoguesLine.size() :
		spearker_name.text = charLine[current_dialogue]
		if dialoguesLine[current_dialogue] == "[INPUT]" and useTyping :
			dialogText.hide()
			...
			if useTyping :
				...
				dialogText.show()
			if dialoguesLine[current_dialogue] == "[RESULT]" and useTyping :
				dialoguesLine[current_dialogue] = typingContainer.text
